select tp.Gestion, tp.Semana, vt.VENTATOTAL 'Monto Vendido', mt.metaGeneral 'Meta Ventas', (vt.VENTATOTAL)/(MT.metaGeneral)*100 '% Logrado',
	tp.Credito 'Ventas al Credito', tp.Contado 'Ventas al Contado', vt.VPTGENERAL 'VTP Logrado', MTTCVPT.VPT_META 'VTP META', (((vt.VPTGENERAL)/(MTTCVPT.VPT_META))*100) '% VTP Logrado', vptmet.UPT 'UPT Logrado'
from (select sum(Monto_real)*0.87 Credito, tb1.Contado Contado, Gestion, Semana
	from TB_VISTA_COMERCIAL tb2,
		...
		where Gestion=DATEPART(year,getdate()-28) and Semana=DATEPART(week,getdate()-28) and Anulado=0 and TipoVenta not like 'Crédito') as tb1
	where Gestion=DATEPART(year,getdate()-28) and Semana=DATEPART(week,getdate()-28) and Anulado=0 and TipoVenta not like 'Contado'
	...
	where M_Gestion_g=DATEPART(year,getdate()-28) and semana=DATEPART(week,getdate()-28)
