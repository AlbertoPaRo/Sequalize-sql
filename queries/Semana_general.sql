...
from (select sum(Monto_real)*0.87 Credito, tb1.Contado Contado, Gestion, Semana
	from TB_VISTA_COMERCIAL tb2,
		...
		where Gestion=DATEPART(year,getdate()-14) and Semana=DATEPART(week,getdate()-14) and Anulado=0 and TipoVenta not like 'Crédito') as tb1
	where Gestion=DATEPART(year,getdate()-14) and Semana=DATEPART(week,getdate()-14) and Anulado=0 and TipoVenta not like 'Contado'
	...
	from TB_VISTA_COMERCIAL
	where Gestion=DATEPART(year,getdate()-14) and Semana=DATEPART(week,getdate()-14) and Anulado=0
	...
	where M_Gestion_g=DATEPART(year,getdate()-14) and semana=DATEPART(week,getdate()-14)
